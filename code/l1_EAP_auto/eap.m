function output = eap(imgr,tmpr, u,func)
img = double(imgr);
[height, width, ~]=size(img); 
eps_weight = 1.0;

k = 2;
output = double(tmpr);
value = sum((output - img).^2,3);
weight = sum((output - imboxfilt(output,3)).^2,3);
knapsack = value./(weight + eps_weight);
thre = sort(knapsack(:));
thre = thre(int64(double(height*width)*(1.0 - double(k) * u / 5.0)));
knapsack = knapsack./thre;
mask = ones(height, width);
alpha = double(5 - k)./4.0;
mask(rand(height, width).*alpha +1.0-alpha<knapsack) = 255;
mask(mask < 255) = 0;
imwrite(mask, strcat('./tmp/', 'mask', num2str(k),'.png'));
imwrite(tmpr, strcat('./tmp/', 'output',num2str(k),'.png'));

for k=3:5
    disp(k);
    output=func(img, repmat(mask, [1, 1, 3]));
    imwrite(mask, strcat('./tmp/', 'mask', num2str(k),'.png'));
    imwrite(output, strcat('./tmp/', 'output',num2str(k),'.png'));
    output = double(output);
    value = sum((output - img).^2,3);
    weight = sum((output - imboxfilt(output,3)).^2,3);
    knapsack = value./(weight + eps_weight);
    thre = sort(knapsack(:));
    thre = thre(int64(double(height*width)*(1.0 - double(k) * u / 5.0)));
    knapsack = knapsack./thre;
    mask = ones(height, width);
    alpha = double(5 - k)./4.0;
    mask(rand(height, width).*alpha +1.0-alpha<knapsack) = 255;
    mask(mask < 255) = 0;
    toc
end
output = uint8(output); % add this line for fix (see issue # 5)
end

